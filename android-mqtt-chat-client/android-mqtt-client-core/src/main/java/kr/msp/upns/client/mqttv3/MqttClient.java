 ...
 */
package kr.msp.upns.client.mqttv3;
...
import kr.msp.upns.client.mqttv3.internal.ClientComms;
import kr.msp.upns.client.mqttv3.internal.DestinationProvider;
import kr.msp.upns.client.mqttv3.internal.ExceptionHelper;
import kr.msp.upns.client.mqttv3.internal.LocalNetworkModule;
import kr.msp.upns.client.mqttv3.internal.MemoryPersistence;
import kr.msp.upns.client.mqttv3.internal.NetworkModule;
import kr.msp.upns.client.mqttv3.internal.SSLNetworkModule;
import kr.msp.upns.client.mqttv3.internal.TCPNetworkModule;
import kr.msp.upns.client.mqttv3.internal.comms.MqttDirectException;
import kr.msp.upns.client.mqttv3.internal.security.SSLSocketFactoryFactory;
import kr.msp.upns.client.mqttv3.internal.trace.Trace;
import kr.msp.upns.client.mqttv3.internal.wire.MqttConnect;
import kr.msp.upns.client.mqttv3.internal.wire.MqttDisconnect;
import kr.msp.upns.client.mqttv3.internal.wire.MqttPingReq;
import kr.msp.upns.client.mqttv3.internal.wire.MqttPubRec;
import kr.msp.upns.client.mqttv3.internal.wire.MqttPublish;
import kr.msp.upns.client.mqttv3.internal.wire.MqttSubscribe;
import kr.msp.upns.client.mqttv3.internal.wire.MqttUnsubscribe;
...
public class MqttClient implements DestinationProvider {
	...
	public MqttClient(String serverURI, String clientId, MqttClientPersistence persistence) throws MqttException {
		if (clientId == null || clientId.length() == 0 || clientId.length() > 50) {
	...
	protected NetworkModule createNetworkModule(String address, MqttConnectOptions options) throws MqttException {
		...
		case URI_TYPE_SSL:
			shortAddress = address.substring(6);
			host = getHostName(shortAddress);
			port = getPort(shortAddress, 8443);
    ...
    public void publish(String topic, byte[] payload, int qos, boolean retain)  throws MqttException, MqttSecurityException {
        MqttMessage mqttMessage = new MqttMessage();
        mqttMessage.setQos(qos);
        mqttMessage.setPayload(payload);
        mqttMessage.setRetained(retain);
        MqttPublish mqttPublish = new MqttPublish(topic,mqttMessage);
        comms.sendAndWait(mqttPublish);
    }

    public void publish(String topic, byte[] payload, int qos, boolean retain, int messageID)  throws MqttException, MqttSecurityException {
        MqttMessage mqttMessage = new MqttMessage();
        mqttMessage.setQos(qos);
        mqttMessage.setPayload(payload);
        mqttMessage.setRetained(retain);
