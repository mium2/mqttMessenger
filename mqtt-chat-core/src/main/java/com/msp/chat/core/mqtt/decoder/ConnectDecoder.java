...
public class ConnectDecoder extends DemuxDecoder {
    ...
    void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws UnsupportedEncodingException {
        ...
        //int keepAlive = ByteUtils.readWord(in);
        ...
        //Decode the ClientID
        String clientID = ByteUtils.decodeString(in);
        if (clientID == null) {
            in.resetReaderIndex();
            return;
        }
        ...
        //Decode willTopic
        if (willFlag) {
            String willTopic = ByteUtils.decodeString(in);
            if (willTopic == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode willMessage
        if (willFlag) {
            String willMessage = ByteUtils.decodeString(in);
            if (willMessage == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode username
        if (userFlag) {
            String userName = ByteUtils.decodeString(in);
            if (userName == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode password
        if (passwordFlag) {
            String password = ByteUtils.decodeString(in);
            if (password == null) {
                in.resetReaderIndex();
                return;
            }
